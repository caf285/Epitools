...
def main():
  ...
  gas = read("/scratch/cfrench/test/zika-tutorial/data/metadata.tsv").split("\n")
  ...
  DIR = "/scratch/GAS/nasp/M82/"
  template = read("/scratch/GAS/.templates/TEMPLATE-M1-config.xml").split("=====")
  ref = subprocess.Popen("ls /scratch/GAS/reference/M82-*", universal_newlines=True, shell=True, stdout=subprocess.PIPE)
  ...
  for path in M1:
    ...
    for sample in M1[path]:
      files.append("            <ReadPair sample=\"" + sample[cols["strain"]] + "\">")
  ...
  if files:
    ...
    write(DIR + "M82-config.xml", "".join(list(map(lambda x: "".join(list(x)), list(zip(template, config)))) + [template[-1]]))
