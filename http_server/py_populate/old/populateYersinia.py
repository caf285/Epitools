...
def main():
  ...
  for table in ["pathogen"]:
    print("\npopulating '" + table + "' ...")
    if table in tableList and exists("./tables/" + table + ".tsv"):
      print("yes")
##### pathogen
      if table == "pathogen":
        # DB
        ...
        # CSV
        csv = list(map(lambda x: x.split("\t"), read("./tables/" + table + ".tsv").strip().split("\n")))
        csvHeader = csv.pop(0)
        ...
        csvEntries = list(map(lambda x: x[:-1] + [json.loads(x[-1])], csvEntries))
        ...
        # EXECUTE
        if csvEntries:
          #sql = "UPDATE " + table + " SET (" + ", ".join(csvHeader) + ") VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
