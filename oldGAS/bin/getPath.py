...
def main():
  ...
  gas = list(map(lambda x: x.split("\t"), read("/scratch/EPITOOLS/GAS/GAS.tsv").strip().split("\n")))
  header = gas.pop(0)

  # get ALL references, strip out the sample names, filter from GAS headers, get new and old ref lists
  allRef = list(filter(lambda x: ".fasta" in x and "ALL::" in x and x.index("ALL") == 0, os.listdir("/scratch/EPITOOLS/GAS/reference")))
  ...
  for ref in noRef:
    if os.path.exists("/scratch/EPITOOLS/GAS/nasp/ALL::" + ref):
      subprocess.call("rm -r /scratch/EPITOOLS/GAS/nasp/ALL::" + ref, universal_newlines=True, shell=True)
  ...
  for ref in list(filter(lambda x: os.path.exists("/scratch/EPITOOLS/GAS/reference/ALL::" + x + ".fasta"), newRef)):
    header.append(ref)
