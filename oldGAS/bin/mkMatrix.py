...
def printHelp():
  ...
  print("example:\n\t./mkMatrix.py /scratch/EPITOOLS/GAS/nasp/All/")
  exit(0)
...
def main():
  ...
  template = read("/scratch/EPITOOLS/GAS/.templates/TEMPLATE_dto.xml").split("=====")
  ...
  if tsv:
    ...
    gas = list(map(lambda x: x.split("\t")[0], read("/scratch/EPITOOLS/GAS/GAS.tsv").strip().split("\n")[1:]))
    print(gas)
    vcf = list(filter(lambda x: x in gas, vcf))
  ...
  if allRef:
    ...
    allRef = allRef[0]
    clade = subprocess.Popen("/scratch/EPITOOLS/GAS/bin/parseNWK.py -c /scratch/EPITOOLS/GAS/nasp/ALL::" + allRef + "/matrices/" + tsv + "/bestsnp.nwk", universal_newlines=True, shell=True, stdout=subprocess.PIPE)
