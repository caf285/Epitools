...
def printHelp():
  ...
  print("example:\n\t./getClades.py /scratch/EPITOOLS/GAS/nasp/ALL/matrices/tree.nwk\n")
  exit(0)
...
def main():
  ...
  #----- build output JSON by known M-types in /scratch/EPITOOLS/GAS/reference
  # extract sample names and mTypes in reference fasta files
  references = list(map(lambda x: x.split(".fasta")[0].split("::") if x.split(".fasta")[-1] == "" else "", os.listdir("/scratch/EPITOOLS/GAS/reference")))
  while '' in references:
    references.remove('')
