...
# ==================================================( functions )
def printHelp():
  print("\ncreates a NASP config file in /scratch/EPITOOLS/GAS/.temp/<ref>-<rand>-config.xml")
  ...
  print("example:\n\t./mkConfig.py /scratch/EPITOOLS/GAS/reference/M1::TG92300.fasta TG92300 -m 80\n")
  exit(0)
...
def main():
  ...
  if not os.path.exists(ref):
    ...
    gatk = list(map(lambda x: x.split("-bwamem-gatk")[0], list(filter(lambda x: len(x.split(".vcf")) == 2 and x.split(".vcf")[-1] == "", os.listdir("/scratch/EPITOOLS/GAS/nasp/" + ref + "/gatk/")))))
  ...
  template = read("/scratch/EPITOOLS/GAS/.templates/TEMPLATE-config.xml").split("=====")
