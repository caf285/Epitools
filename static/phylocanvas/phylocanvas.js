...
/***/ function(module, exports, __webpack_require__) {
	...
	var Branch = function () {
	  ...
	  _createClass(Branch, [{
        ...
        // halves the size of the internal nodes bouded radius
        if (!this.leaf) {
	        boundedRadius *= 2 / 3;
        }
        ...
        // 7GLS - get label start x
        // always constant, turn off x adjust on highlight
...
/***/ function(module, exports) {
	...
	exports.default = {
      ...
      // 12FIN - fill internal nodes
	    node.canvas.beginPath();
      if (node == tree.root) {
	      node.canvas.moveTo(node.startx-100, node.starty);
	      node.canvas.lineTo(node.startx, node.starty);
	      node.canvas.stroke();
      }
      // check if node or sibling node is selected for highlight
      if (node.parent) {
        var tog = true;
        for (var child in node.parent.children) {
          if (node.parent.children[child].selected) {
            tog = false;
          }
        }
        if (tog) {
  	      node.canvas.fillStyle = "#000000";
        }
      }
      node.canvas.arc(node.startx, node.starty, tree.baseNodeSize * 2 / 3, 0, 2 * Math.PI, true)
	    node.canvas.fill();
	    node.canvas.closePath();
      //
